//! Downstream-facing `Contract` accessor for the `istmo.live_activity`
//! plugin.
//!
//! Enabled via the `codegen` feature. Downstream apps that need to
//! codegen Kotlin / Swift host classes from `build.rs` add
//! `istmo-live-activity = { workspace = true, features = ["codegen"] }`
...
//! consumer-build time via [`istmo_build::extract_contract`], so the
//! trait declaration in `lib.rs` remains the single source of truth.
...
/// Extracts the `LiveActivity` contract from `src/lib.rs`.
///
/// # Panics
