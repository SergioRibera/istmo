...
plugins {
    ...
    // from the enclosing Cargo workspace and injects it into this
    // module's Kotlin source set — no manual copy, no `srcDirs` list.
...
android {
    ...
    defaultConfig {
        applicationId = "dev.istmo.sharedemo"
        minSdk = 26
        targetSdk = 34
        versionCode = 1
        versionName = "0.1"
...
istmoCargoLib("share-demo")

dependencies {
    implementation("dev.istmo:istmo-runtime:0.1.0")
    // Declared by istmo-share's `istmo.toml`: FileProvider and
    // ShortcutManagerCompat (direct share).
